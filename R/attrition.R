...
#' @param inputs Numeric vector of n's going in
#' @param losses Numeric vector of n's going out - last is those included
#' @param unit Preface a unit?
#' @param labels Character vector of labels - in order of inputs then losses.
#' @param format The default is "plot," which produces a plot in the R graphics device. Current alternate options include "pdf" and "bmp," which produce those file types under the name "Sankey.xxx" in the current directory.
#' @keywords teradata attrition sankey
#' @export
#' @importFrom magrittr "%>%"
#' @examples
#' attrition(
...
attrition <- function(
  ...
  inputLabel = paste(labels[1],": ",unit,inputs[1]," (",round(100*frInputs[1],digits=1),"%)",sep="")
  lossLabel = paste(labels[length(labels)],": ",unit,losses[length(losses)]," (",round(100*frLosses[length(losses)],digits=1),"%)",sep="")
  ...
  if(length(inputs)>1){
    for(j in 2:length(inputs)){
      ...
      fullLabel = paste(labels[j],": ",unit,inputs[j]," (",round(100*frInputs[j],digits=1),"%)",sep="")
      fontsize = max(0.8,frInputs[j]*1)
  ...
  for(i in 1:(length(losses)-1)){
    ...
    fullLabel = paste(labels[i+length(inputs)],": ",unit,losses[i]," (",round(100*frLosses[i],digits=1),"%)",sep="")
    fontsize = max(0.6,frLosses[i]*1)
    text(txtX, txtY, fullLabel, cex=fontsize, pos=4, srt=45, offset=0.1)
  ...
  # Last loss label
  lossLabel = paste(labels[length(labels)],": ",unit,losses[length(losses)]," (",round(100*frLosses[length(losses)],digits=1),"%)",sep="")
  fontsize = max(0.8,frLosses[length(losses)]*1)
